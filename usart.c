...
void openmv_uart_init(u32 bound){  // openmv串口初始化
   //GPIO端口设置
   ...
   //USART1 初始化设置
	...
	//Usart1 NVIC 配置
	NVIC_InitStructure.NVIC_IRQChannel = USART1_IRQn;//串口1 OPENMV
...
void USART1_IRQHandler(void)                	//串口1中断服务程序
{
	...
	{
		openmv_rx((unsigned char)USART1->DR);//处理数据
...
void gyro_uart_init(u32 bound){ //陀螺仪串口初始化
   //GPIO端口设置
	GPIO_InitTypeDef GPIO_InitStructure;
	USART_InitTypeDef USART_InitStructure;
	NVIC_InitTypeDef NVIC_InitStructure;
   ...
   //USART3 初始化设置
	...
	//Usart3 NVIC 配置
	NVIC_InitStructure.NVIC_IRQChannel = USART3_IRQn;//串口3中断通道 陀螺仪
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority=0;//抢占优先级1
	NVIC_InitStructure.NVIC_IRQChannelSubPriority =0;		//子优先级1
	NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;			//IRQ通道使能
	NVIC_Init(&NVIC_InitStructure);	//根据指定的参数初始化VIC寄存器
...
void USART3_IRQHandler(void)                	//串口3中断服务程序 陀螺仪串口初始函数
{

	if(USART_GetITStatus(USART3, USART_IT_RXNE) != RESET)//接收数据
	{
		Gyro_Uart_Rx((unsigned char)(USART3->DR));//处理数据
...
void hc_uart_init(u32 bound){  // 外接蓝牙串口初始程序
   //GPIO端口设置
	GPIO_InitTypeDef GPIO_InitStructure;
	USART_InitTypeDef USART_InitStructure;
	NVIC_InitTypeDef NVIC_InitStructure;
	...
	//串口3对应引脚复用映射
	GPIO_PinAFConfig(GPIOC,GPIO_PinSource10,GPIO_AF_UART4); //GPIOB10复用为USART1
	GPIO_PinAFConfig(GPIOC,GPIO_PinSource11,GPIO_AF_UART4); //GPIOB11复用为USART1
